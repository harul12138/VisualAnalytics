...
function showTooltip(d) {
	...
	if (inSearch == true) {
		//Don't do anything if the search is active and the user doesn't mouse over the selected title
		if (d.title.toLowerCase() !== selectedtitle.toLowerCase()) return;	
	} else {
	...
	d3.select("#tooltip .tooltip-band").text(d.title);
	d3.select("#tooltip-year").html(d.yearr);
