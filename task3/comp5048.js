...
//	&& k != 'Display Diagonal (in)' 
	...
	&& k != 'Volume (cubic cm)' 
//	&& k != 'Mass (grams)' 
//	&& k != 'Pixel Density (per inch)' 
	&& k != 'Cluster ID'
//	&& k != 'Screen to Body Ratio'
	) {
...
function render_range(selection, i, max, opacity) {
  ...
  for(i = 0; i < range.length; i++) {
    d = range[i]
    path(d, foreground, color(d["Cluster ID"], opacity));
  }
};
...
function data_table(sample) {
  ...
  table.append("span").attr("class", "color-block").style("background", function(d) {
    return color(d["Cluster ID"], 0.85)
  })
...
function highlight(d) {
  ...
  d3.selectAll(".row").style("font-weight", function(p) {
    return(d["Cluster ID"] == p) ? "bold" : null
  }).style("opacity", function(p) {
    return(d["Cluster ID"] == p) ? null : "0.3"
  });
  path(d, highlighted, color(d["Cluster ID"], 1));
}
...
function brush() {
  ...
  if (!hx.brush.empty()) {
    actives.push("Release Year");
    extents.push(hx.brush.extent());
  }
  ...
  d3.selectAll('.label').style("fill", function(dimension) {
    if(_.include(actives, dimension)) return "#FF0000";
    return null;
  });
  ...
  filtered_data = data.filter(function(d) {
    return !_.contains(excluded_groups, d["Cluster ID"].toString());
  })
  ...
  legend.selectAll(".color-bar").style("width", function(d) {
    return Math.ceil(width * 0.15 * tallies[d].length / (maximumLength + 1)) + "px"
  });
...
function paths(selected, ctx, count) {
  ...
  //data_table(selected);
  data_table(shuffled_data.slice(0, 24));
...
d3.select("#theme").on("click", dark_theme);
d3.select("#remove-filters").on("click", resetFilters);
d3.select("#sel-all").on("click", unselectAll);
...
function unselectAll() {
  ...
  d3.selectAll("#sel-all").on("click", selectAll).text("Select All Categories");
  brush();
}

function selectAll() {
  excluded_groups = [];
  d3.selectAll("#sel-all").on("click", unselectAll).text("Unselect All Categories");
  brush();
}
...
function dark_theme() {
  ...
  d3.selectAll("#theme").on("click", light_theme).text("Light Theme");
}

function light_theme() {
  d3.select("body").attr("class", null);
  d3.selectAll("#theme").on("click", dark_theme).text("Dark Theme");
}
...
function search(selection, str) {
  ...
  return _(selection).filter(function(d) {
    return pattern.exec(d.Model);
  });
}

/******************************************************************************************************************/

d3.select("#chartlabel").style("width", document.body.clientWidth);

d3.select("#chart2").style("height", (h + m[0] + m[2]) + "px");

var hmargin = {top: 20, right: 30, bottom: 30, left: 30},
    hwidth = document.body.clientWidth - hmargin.left - hmargin.right,
    hheight = 500 - hmargin.top - hmargin.bottom;


var hsvg = d3.select("#chart2").append("svg")
...
var bars = hsvg.append("g")
    .attr("class", "bars");

var xmin = 2002, xmax = 2014
...
function hbrushx() {
  ...
  d3.select("#chart2").select("svg").selectAll(".hbrush").remove();
  
  // Add and store a brush for each xAxis.
  hsvg.append("g").attr("class", "hbrush").attr("transform", "translate(0," + hheight + ")").call(hx.brush = d3.svg.brush().x(hx).on("brush", brush)).selectAll("rect").style("visibility", null).attr("y", -15).attr("height", 30).append("title").text("Drag left or right to brush along this axis");

  hsvg.selectAll(".extent").append("title").text("Drag or resize this filter");

}


d3.selectAll(".hxaxis g").style("display", "inline");




function hbrush(data) {

  // Generate a log-normal distribution with a median of 30 minutes.
  var values = [];//d3.range(1000).map(d3.random.logNormal(Math.log(300), .4));
  
  for (i = 0; i < data.length; i++) {
    {values[i] = data[i]["Release Year"];}
  }
  ...
  d3.select("#chart2").select("svg").selectAll(".bar").remove();
  ...
  bar.append("rect")
     .attr("x", 1)
     .attr("width", hx(hdata[0].dx + xmin/*d3.min(values)*/) - 1)
     .attr("height", function(d) { return hheight - hy(d.y); });

  bar.append("text")
     .attr("dy", ".75em")
     .attr("y", -15)
     .attr("x", hx(hdata[0].dx + xmin/*d3.min(values)*/) / 2)
     .attr("text-anchor", "middle")
     .text(function(d) { return formatCount(d.y); });
}

// Highlight averageLine

function avghighlight() {
  d3.select("#foreground").style("opacity", "0.25");
  d3.select("#avgdemo").style("font-weight", "bold");
  path(averageLine, highlighted, color("Average", 1))
}

// Remove avghighlight

function avgunhighlight() {
  d3.select("#foreground").style("opacity", null);
  d3.select("#avgdemo").style("font-weight", null).style("opacity", null);
  highlighted.clearRect(0, 0, w, h);
}


d3.select("#avgdemo").select("strong").style("color",colors["Average"]);

d3.select("#avgdemo").on("mouseover", avghighlight).on("mouseout", avgunhighlight);
...
function drop(event) { 
    ...
    var dm = document.getElementById('wrap');
